...
module PdkSync
  module Utils
    ...
    def self.return_gems
      raise "File '#{configuration.managed_gems}' is empty/does not exist" if File.size?(configuration.managed_gems).nil?
      YAML.safe_load(File.open(configuration.managed_gems))
    end
    ...
    def self.validate_gems_exist(client, gem_names)
      raise "Error reading in gem. Check syntax of '#{configuration.managed_gems}'." unless !gem_names.nil? && gem_names.is_a?(Array)
      invalid = gem_names.reject { |name| client.repository?("#{configuration.namespace}/#{name}") }
      # Raise error if any invalid matches were found
      raise "Could not find the following repositories: #{invalid}" unless invalid.empty?
      true
    end
